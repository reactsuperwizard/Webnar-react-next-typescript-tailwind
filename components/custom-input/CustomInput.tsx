...
export const CustomInput = React.forwardRef<HTMLInputElement, InputFormProps>(
  (
    {
      type = "text",
      state = "normal",
      ...
      <div {...props}>
        ...
        <div tw="relative">
          <InputBodyWrapper darkmode={variant} iconType={iconType}>
            ...
            {error && (
              <div tw="text-fireopal-400 text-xs sm:text-sm py-1">{error}</div>
            )}
          </InputBodyWrapper>
...
const CustomText = styled(TextField)<{
  ...
  padding: ${({ isIcon, sizeType }) =>
    sizeType === "sm"
      ...
      : "14px 16px"};
  ${({ darkmode }) =>
    darkmode ? tw`bg-[#FAFAFE] text-white` : tw`bg-[#FAFAFE] text-gray-900`}

  &:focus {
    ${tw`!outline-none`}
    border-color: ${({ state, darkmode }) =>
      state === "success"
        ? darkmode === "light"
          ? "#1E6759"
          : "#2BA185"
  ...
  ${({ state }) =>
    state === "danger"
      ? tw`border-fireopal-500`
      : state === "success"
      ? tw`border-gogreen-500`
      : tw`border-csp_neutral-contrast1`}
  ...
  ${({ darkmode }) =>
    darkmode === "dark" ? tw`text-[#A1A1AA]` : tw`text-gray-900`}
  ${({ state }) =>
    state === "danger"
      ? tw`text-fireopal-500`
      : state === "success"
      ? tw`text-gogreen-500`
      : tw`text-gray-900`}
