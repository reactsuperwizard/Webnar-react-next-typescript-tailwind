...
type BlogProps = {
  blog?: BlogPost;
  type: "latest" | "featured" | "normal";
  onClick?: () => void;
};

export const Blog = ({ blog, type, onClick }: BlogProps) => {
  return (
    <BlogContainer type={type}>
      ...
      <BlogContent type={type}>
        {type !== "featured" && (
          <span tw="inline-block  mb-[15px] bg-[#FFE688] rounded-[6px] text-[12px] px-[10px] text-[#723B13] font-semibold">
            Category
          </span>
        )}
  ...
  ${({ type }) =>
    type === "latest"
      ...
      : type === "normal"
        ...
        : tw`relative w-[100%] h-[240px] lg:h-[346px] rounded-[8px]`}
`;
